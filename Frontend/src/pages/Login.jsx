...
import { nameValidator, useremailValidator, usernameValidator } from "../utils/validators";
...
const Login = () => {
  ...
  const useremail = useInputValidation("", useremailValidator);
    ...
    <div style={{ backgroundImage: bgGradient }} >
      <Container component={"main"} maxWidth="xs" sx={{ height: "100vh", display: "flex", justifyContent: "center", alignItems: "center" }} >
        <Paper elevation={3} sx={{ padding: 4, display: "flex", flexDirection: "column", alignItems: "center" }} >
          {isLogin ? (
            <>
              <Typography variant="h5">SignIn User</Typography>
              <form style={{ width: "100%", marginTop: "1rem" }} onSubmit={handleLogin} >
                ...
                <Button sx={{ marginTop: "1rem" }} variant="contained" color="primary" type="submit" fullWidth disabled={isLoading}>SignIn User</Button>
                <Typography textAlign={"center"} m={"1rem"}> OR</Typography>

                <Button disabled={isLoading} fullWidth variant="text" onClick={toggleLogin}>Don't Have Account! Click Here</Button>
              </form>
            </>
          ) : (
            <>
              <Typography variant="h5">SignUp User</Typography>
              <form style={{ width: "100%", marginTop: "1rem" }} onSubmit={handleSignUp} >
                ...
                <TextField required fullWidth label="Email" type="email" margin="normal" variant="outlined" value={useremail.value} onChange={useremail.changeHandler} />
                {useremail.error && ( <Typography color="error" variant="caption"> {useremail.error} </Typography> )}

                <TextField required fullWidth label="Password" type="password" margin="normal" variant="outlined" value={password.value} onChange={password.changeHandler} />

                <Button sx={{ marginTop: "1rem" }} variant="contained" color="primary" type="submit" fullWidth disabled={isLoading}> SignUp User </Button>

                <Typography textAlign={"center"} m={"1rem"}> OR </Typography>

                <Button disabled={isLoading} fullWidth variant="text" onClick={toggleLogin}> Already Have An Account! Click Here </Button>
              </form>
            </>
          )}
        </Paper>
