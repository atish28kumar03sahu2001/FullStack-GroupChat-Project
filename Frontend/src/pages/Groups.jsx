...
import { bgGradient, matBlack, purpleColor } from "../constants/color";
      ...
      <Grid item xs={12} sm={8} sx={{ display: "flex", flexDirection: "column", alignItems: "center", position: "relative", padding: "1rem 3rem" }}>
        ...
        {groupName && (
          <>
            ...
            <Typography fontSize={"20px"} color={purpleColor} margin={"2rem"} alignSelf={"flex-start"} variant="body1" > Members </Typography>
...
const GroupsList = ({ w = "100%", myGroups = [], chatId }) => (
  <Stack width={w} sx={{ height: "100vh", overflow: "auto", backgroundColor: purpleColor }} >
    {myGroups.length > 0 ? (
      myGroups.map((group) => (
        <GroupListItem group={group} chatId={chatId} key={group._id} />
      ))
    ) : (
      <Typography textAlign={"center"} fontSize={"20px"} padding="1rem"> No groups </Typography>
    )}
  </Stack>
);
  ...
  return (
    <Link to={`?group=${_id}`} onClick={(e) => { if (chatId === _id) e.preventDefault(); }} >
      <Stack direction={"row"} spacing={"1rem"} alignItems={"center"}>
        ...
        <Typography color={"white"} fontSize={"14px"}>{name}</Typography>
      </Stack>
    </Link>
  );
});
